/*
    ...
    일반 SQL의 단점을 보완해서 변수의 정의, 조건(IF), 반복(FOR, WHILE)등을 지원하여 다수의
    SQL문을 한번에 실행할 수 있음.
    PL/SQL은 블록구조를 사용함.
    
    PL/SQL 구조
    DECLARE     -- (선택)선언부 : 변수, 산수등을 선언
...
BEGIN
    ...
    DBMX_OUTPUT.PUT_LINE(*EID : || EID);
    DBMX_OUTPUT.PUT_LINE(*EID : || ENAME);
    DBMX_OUTPUT.PUT_LINE(*EID : || EIPI);
END;
/
...
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    NCODE EMPLOYEE.NATIONAL_CODE%TYPE;
BEGIN
...
BEGIN
    FOR I IN 1..100
    LOOP
        INNSERT INTO TEST VALUES(SEQ_TNO.NEXTVAL, SYSDATE);
    END LOOP;
END;
/
...
BEGIN
    ...
    WHERE EMP_NAME = '노옹철';
EXCEPTION
    WHEN ZERO_DIVEDE THEN DBMS.PUT_LINE('나누기 연산자는 0으로 나눌 수 없습니다');
