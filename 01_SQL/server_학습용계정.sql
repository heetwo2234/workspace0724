...
SELECT SUBSTR('SHOWMETHEMONEY', 5, 2) FROM DUAL; -- 5번위치까지 가서 두개만 추출
SELECT SUBSTR('SHOWMETHEMONEY', 1, 6) FROM DUAL; -- 1번위치부터 6번위치까지
SELECT SUBSTR('SHOWMETHEMONEY', -8, 3) FROM DUAL; -- -8번위치부터 3번위치까지

SELECT EMP_NAME, EMP_NO, SUBSTR(EMP_NO, 8, 1) AS "성별"
FROM EMPLOYEE;

-- 사원들 중 여자사원들만 EMP_NAME(이름), EMP_NO(주민등록번호)를 조회
SELECT EMP_NAME, EMP_NO
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 8, 1) IN '2';

-- 사원들 중 남자사원들만 EMP_NAME(이름), EMP_NO(주민등록번호)를 조회
SELECT EMP_NAME, EMP_NO
FROM EMPLOYEE
...
SELECT LTRIM('ACABACCAKHBA', 'ABC') FROM DUAL;
...
SELECT LOWER('Welcome To My KH') FROM DUAL; --소문자로
SELECT UPPER('Welcome To My KH') FROM DUAL; --대문자로
SELECT INITCAP('Welcome To My KH') FROM DUAL; --띄어쓰기 할때마다 대문자

---------------------------------------------------------------------
/*
    *CONCAT
    문자열 두개 전달받아서 하나로 합친 후 변환
    CONCAT(STR1, STR2)
*/
...
SELECT EMP_NAME, HIRE_DATE, FLOOR(SYSDATE - HIRE_DATE), CEIL(MONTH_BETWEEN(SYSDATE, HIRE_DATE))
FROM EMPLOYEE;
...
/*
    <형변환 함수>
    *TO_CHAT : 숫자타입 또는 날짜타임의 값을 문자타입으로 변환시켜주는 함수
...
SELECT TO_CHAR(SYSDATE, 'PHH:MI:SS') FROM DUAL;
SELECT TO_CHAR(SYSDATE, 'YYYY-MM-DD DAY HH:MI:SS') FROM DUAL;

--년 월 일 시 분 초 하려면 아래처럼
...
--월과 관련된 포맷
SELECT TO_CHAR(SYSDATE, 'MM'), -- 오늘이 이번년도 몇번째 일수
        TO_CHAR(SYSDATE, 'MON'), -- 오늘일자
        TO_CHAR(SYSDATE, 'MONTH') -- 요일
FROM DUAL;
...
SELECT TO_DATE(611218) FROM DUAL;

SELECT TO_DATE(051010) FROM DUAL; -- 0으로 시작하는 숫자는 없다 x
...
/*
TO_NUMBER : 문자타입의 데이터를 숫타입으로 변환해서 반환
...
--NULLIF(비교대상1, 비교대상2)
--두값이 일치하면 NULL반화느 일치하지 않으면 비교대상1 반환
...
먼저 실행되는 순서
FROM 테이블 -> WHERE 조건식(어떤 필터링을 할지)
-> SELECT *(어떤걸 보여줄건지에 대한 구조를 잡는다)
-> ORDER BY 정렬기준
*/
