...
-- 서브쿼리 예시1) 노옹철 서원과 같은 부서에 속한 사원들 전체 조회
SELECT *
FROM EMPLOYEE
...
SELECT EMP_NAME, EMP_ID, DEPT_TITLE, SALARY
FROM EMPLOYEE
...
/*
    ...
    ANY:이 중에 하나라도 너보다 큰게있어?, 이 중에 하나라도 너보다 작은게 있어?
...
--2) 대리직금임에도 과장직급의 최소급여보다 더 많이 받는 사원들의 사번, 이름, 직급, 급여 조회
...
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
                                ...
                                WHERE EMP_NAME = '하이유');
--컬럼 순서가 동일해야됨
--DEPT_CODE, JOB_CODE = DEPT_CODE, JOB_CODE
...
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE (JOB_CODE, SALARY) IN (SELECT JOB_CODE, MIN(SALARY_
                                FROM EMPLOYEE
                                GROUP BY JOB_CODE);
...
--부서코드, 부서별 총 급여함, 부서별 평균급여, 부서별 사원수 조회
SELECT DEPT_CODE, SUM(SALARY), AVG(SALARY), COUNT(*)
FROM EMPLOYEE
--DEPT_CODE 안에 SUM AVG COUNT 다 넣는다
GROUP BY DEPT_CODE
